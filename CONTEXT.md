...
**口表语域（oral register）**:
五个教练 skill 全部输出共用的说话底色：像当面说话、不像伏案写作，取中间一档——比口语实录收敛（废话全删）、比书面文章直接（不铺垫不绕弯），陪练对话、快照行、手写产物三面通用。定义单源 `skills/references/voice.md` §1（六条正行为 + 先删后改 + 念出声检验法）；教学场景的专属收紧（语气词限量、正文自然段、模板套话一刀切）另见 coach.md §3.5，是对通用判真的收紧不是另一套规则。
_Avoid_: 写作腔/讲义腔（那是它要治的病）、满篇口水话（中间档不是照搬口语实录）

**推荐行措辞（recommendation wording）**:
快照与推荐行的话怎么说：动作先行（推荐第一要素是可执行入口 F 编号/命令/按钮，不是态度）、依据可指认（「为什么」必须带实测数字或 EP 考点名其一，有档案错因引原文）、禁空话（万能句与无依据褒贬——换个学习者还成立的理由就是空话）。模板的行结构/表格/emoji/菜单分组是扫读功能不算「话」，不受此辖。定义单源 `skills/references/voice.md` §2；ask-coach Step 2 推荐行与薄命令开场「今天最该练 + 为什么」同受管辖。
_Avoid_: 「错题较多建议复习」（万能句 + 无依据 + 没有动作，三病齐犯）、把数字抹成形容词（「不少题没答」≠「已答 3/10」）

**读者预期**:
对话层纪律：给出判断后预估对方下一秒会问什么（凭什么 / 那 X 算不算 / 具体怎么办），紧接着把答案递上去——或解释、或例子、或边界条件，写成陈述不装问句；十有八九会问的才接，小概率疑问略过。定义单源 `skills/references/voice.md` §3（条款名「预答下一问」）。
_Avoid_: 自问自答（句句设问自答就成了应答手册）、把预估写成问句（直接补陈述即可）

**话语标记判真**:
关节词（首先/所以/其实/总之一类）的处理纪律：通用层不设禁用词表，逐个过四问——删掉后理解路径受损吗、跟别处重复吗、语义已自明吗、造成模板感吗——有真实导航功能的留、没有的删。教学对话另有一刀切禁用的收紧（coach.md §3.5），是四问的便宜近似而非另一套规则。定义单源 `skills/references/voice.md` §3。
_Avoid_: 禁词表当通用规则（那是教学场景收紧，通用层逐词判真）

**口述成文**:
把教学对话整理成笔记 / 范式卡 / 冲刺包的成文法：按读者使用时的认知顺序重组（考前翻它需要什么），不按讲课时间线照搬；压缩重复但保住强调意图与限定分寸——删的是无功能口头禅，不是具体细节与判断温度。定义单源 `skills/references/voice.md` §4（四动作 + 检验法已成文）。
_Avoid_: 讲课记录整理（时间线照搬不是按用途重组）、成文即冷冰冰（删口头禅不等于删判断温度）
