...
**skill（vs CLI）**:
skill 指装进 agent 环境、给 AI 执行的指令包——仓库内共四个：`/ask-coach` 主入口（状态快照 → 推荐 → 带执行，源目录 `skills/ask-coach/`，带 references/ 三份参考文档）+ 三个薄命令 `/coach`（陪练直入）/`/doctor`（体检）/`/recap`（错题串讲直入），薄命令只有十几行 SKILL.md、共享主入口的 references/。**命名史**：原 study-coach → 2026-08-25 改名 ai-study-kit → 2026-09 v0.13 主 skill 更名 ask-coach（插件名 ai-study-kit 终身不变，市集名不可改）。CLI 指仓库 `apps/quiz-app/scripts/` 下的可执行脚本（teach-generate / grill-wrong / podcast-generate）。两类东西，不混称。
