...
按 [`references/state.md`](references/state.md) 的探测协议采集九个字段，拼成「学习状态快照」：
...
6. 未毕业错题 ≥ 3 → **F4 错题串讲**（错题攒够了，值得 LLM 聚类深挖一轮。点名 mastery 报告最前的弱考点，档案里有错因就引用——理由从「错题多」具体到「EP-03 chmod 连错 2 次，错因：权限位组合不熟」；`oral.weakRanked` 里的口头弱项一并点名，无题知识点口头没到掌握线就再抽几轮）
