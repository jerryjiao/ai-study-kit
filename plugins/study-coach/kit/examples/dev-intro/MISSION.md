...
大纲是唯一权威——先定考什么、考多深，再照表产题产卡。**考点列填核心关键词**：四对齐校验（`pnpm run check:alignment`）按它做课程覆盖、闪卡覆盖和题量对账。
...
| EP-04 | 相对路径 | 了解 | judge×1 | D2 | 0 |

题的 `examPoint` 填考点 id、`day` 对齐表的 day 列；闪卡数 0 = 大纲声明该考点不配卡（了解级）。
