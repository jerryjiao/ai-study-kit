...
> ИИ-тренер, который гонит любую тему до настоящего понимания. Вопросы собираете сами или поручаете ИИ; ошибки разбираются по пунктам экзамена, повторения встают в очередь сами, прогресс синхронизируется между устройствами. Бесплатно и с открытым кодом: поставили плагин — и можно начинать.
...
## 📦 Установите в свой ИИ-инструмент

Шаги установки зависят от инструмента, и каждому нужна своя копия. Плагин везёт полный исходный код приложения, клонировать репозиторий не нужно; агент проведёт вас от нуля до работающего учебного сайта, генерируя вопросы и курсы по пути.
...
После установки скажите ИИ «хочу выучить X» или запустите `/ask-coach`. Ваш учебный проект (копия приложения + ваши пакеты тем) целиком живёт в вашем каталоге и переживает обновления плагина. Покрыты тринадцать сценариев под ключ, сгруппированных в четыре линии — обучение (тренировка с наставником, спринт перед экзаменом), практика (ежедневная учёба, разбор ошибок, подкасты), контент (новая тема, генерация курсов, правка контента), эксплуатация (развёртывание, обновление, проверка, деплой, проекция графа знаний) — см. [`docs/ai-study-kit.ru.md`](docs/ai-study-kit.ru.md).

Просто хотите сначала посмотреть? Ниже — маршрут с клонированием.
...
**Одной фразой**: вы приносите вопросы; инструмент превращает их в учебное приложение с курсами, карточками и разбором ошибок.

---

## 🚀 Демо за 5 минут (маршрут с клонированием, для разработчиков)
...
С установленным выше плагином (или через `pnpm run skill:install` в `~/.agents/skills/`) каждое занятие можно начинать отсюда.

**Имена команд — это меню**, всего пять: `/ask-coach` — спросить тренера, что делать (снимок состояния + рекомендация + ведение по шагам), `/study-coach` — сразу в тренировочную сессию, `/study-doctor` — комплексная проверка (ворота качества + зонды окружения), `/study-recap` — сразу в разбор ошибок, `/study-podcast` — превратить учебный материал в аудио с двумя ведущими для дороги; остальное идёт через маршрутизатор.

`/ask-coach` сначала **сканирует ваше учебное состояние** (текущая тема, запасы вопросов/карточек/курсов, прогресс, срок карточек, число ошибок, завершённые уроки, прогресс тренировки, конфигурация ИИ), затем **советует одно самое полезное действие прямо сейчас** — начать новую тему, повторить срок-карточки, порешать вопросы или собрать накопленные ошибки в подробный разбор — а после выбора **ведёт вас по шагам**. Тринадцать сценариев покрывают всё: от инициализации проекта до деплоя. (Файлы сценариев, которые исполняет skill, написаны по-китайски; это руководство — по-русски: [`docs/ai-study-kit.ru.md`](docs/ai-study-kit.ru.md).)
...
> **Не хотите класть контент в репозиторий кита?** Пакет темы может жить в вашем собственном проекте: `cp -r examples/dev-intro /путь/к/вашему-проекту/theme/react-basics`, затем передавайте этот путь везде, где ожидается `EXAMPLE_THEME` (значение с разделителями пути считается внешним пакетом темы — все четыре скрипта его понимают: синк сборки / генерация курсов / разборы ошибок). Репозиторий кита остаётся чистым и используется просто как инструмент — см. [`docs/adr/0004`](docs/adr/0004-external-theme-packs.md).
...
Все поля, включая необязательные `day` (метка учебного дня) и `examPoint` (id экзаменационного пункта — проверка согласованности четвёрки сверяет их с таблицей из MISSION), — в интерфейсе `Question` файла [`apps/quiz-app/src/types.ts`](apps/quiz-app/src/types.ts).
...
python3 scripts/bidirectional-check.py examples/react-basics/  # проверка согласованности четвёрки (сверка с таблицей экзаменационных пунктов из MISSION)
```

**Готово.** Вы ни разу не тронули React-код — только JSON и HTML.

### Пусть ИИ-агент напишет вопросы за вас (необязательно)

Не хотите писать JSON руками? Установите `/ask-coach` (раздел «🧭 Не знаете, за что взяться?» выше) и просто попросите сгенерировать банк вопросов для react-basics. Агент работает по дисциплинированному сценарию: сначала согласует с вами **таблицу экзаменационных пунктов** в MISSION.md — что проверяем, насколько глубоко, сколько вопросов каждого типа, сколько карточек — затем пишет вопросы и карточки пункт за пунктом и в конце сам прогоняет трое ворот качества (`qa` / `scan` / согласованность четвёрки); сдаёт работу только когда всё зелёное. Ручной путь остаётся основным: оба пути дают одинаковые артефакты, а таблица пунктов — контракт между вами и агентом.
...
# подробности в docs/configuration.ru.md
```
...
Полное описание CLI, параметры и FAQ — в [`docs/ai-cli-guide.ru.md`](docs/ai-cli-guide.ru.md). Детали конфигурации — в [`docs/configuration.ru.md`](docs/configuration.ru.md).
...
`--lang` влияет на генерируемый контент и фиксированные тексты HTML (навигация, подвал, `<html lang>`). Логи CLI остаются на китайском; сам банк вопросов не переводится. См. раздел «输出语言» в [`docs/ai-cli-guide.ru.md`](docs/ai-cli-guide.ru.md).
...
|-------------------|-----------------|
| **Anki**: отличные карточки, но нет тренажёра, разборов ошибок и курсов | 6 учебных артефактов в одном приложении, все на одном наборе пунктов экзамена |
...
**Главная отличительная черта**: **цикл согласованности четвёрки** — пункты экзамена, которые объясняют курсы, проверяют вопросы, закрепляют карточки и раскрывают разборы ошибок, — один и тот же набор (см. [docs/four-alignment.ru.md](docs/four-alignment.ru.md)). Закончили урок — соответствующие вопросы уже рядом; ошиблись — разбор в одной команде от вас.
...
| [`docs/theming.ru.md`](docs/theming.ru.md) | настройка отображения: справочник полей theme-config.json |
...
| [`docs/bidirectional-check.ru.md`](docs/bidirectional-check.ru.md) | автоматические перекрёстные проверки |
...
pnpm run skill:install    # установить команды тренера (/ask-coach + тонкие)
...
3. Если меняли какие-либо артефакты (курсы / вопросы / карточки / разборы), прогоните и [`bidirectional-check`](docs/bidirectional-check.ru.md)
...
- Структура контент-пакета (MISSION → RESOURCES → lessons) и дисциплина составления вопросов (равные по длине варианты, без подсказок форматированием) заимствованы из [teach skill Мэтта Покока](https://github.com/mattpocock); история решения — в `docs/adr/0001-agent-authored-questions-not-cli.md`
