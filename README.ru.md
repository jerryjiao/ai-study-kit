...
**Имена команд — это меню**, всего четыре: `/ask-coach` — спросить тренера, что делать (снимок состояния + рекомендация + ведение по шагам), `/coach` — сразу в тренировочную сессию, `/study-doctor` — комплексная проверка (ворота качества + зонды окружения), `/study-recap` — сразу в разбор ошибок; остальное идёт через маршрутизатор.
