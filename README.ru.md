...
**Имена команд — это меню**, всего четыре: `/ask-coach` — спросить тренера, что делать (снимок состояния + рекомендация + ведение по шагам), `/coach` — сразу в тренировочную сессию, `/doctor` — комплексная проверка (ворота качества + зонды окружения), `/recap` — сразу в разбор ошибок; остальное идёт через маршрутизатор.
