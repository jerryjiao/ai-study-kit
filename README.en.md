...
**The command names are the menu** — five in total: `/ask-coach` asks the coach what to do (state snapshot + recommendation + guided execution), `/study-coach` jumps straight into a tutoring session, `/study-doctor` runs a one-stop health check (quality gates + environment probes), `/study-recap` jumps straight into a wrong-question deep-dive, `/study-podcast` turns study material into a two-host audio show for your commute; everything else goes through the router.
