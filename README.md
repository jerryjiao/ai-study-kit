...
学习项目（答题站 + 你的主题包）全部住你自己的目录，与插件升级互不干扰。初始化、开新主题、每日学习、错题串讲、播客、产课、现场串讲、校验、部署十个流程全覆盖，详见 [`docs/ai-study-kit.md`](docs/ai-study-kit.md)。
...
它会先**扫描你的学习状态**（当前主题、题/卡/课库存、答题进度、到期闪卡、错题数、课程已读、AI 配置），然后**推荐你现在最该做的一件事**——是开新主题、复习到期闪卡、刷题、还是把攒下的错题做成串讲——选定后**带着你一步步执行**。从初始化整个项目到部署上线，十个流程全覆盖。
