...
1. **先探测**（只读）：按 [`../references/state.md`](../references/state.md) 跑 Step 1，对齐 F5 前置——AI 配置（`.env` LLM 三项，缺了先给配置指引）、TTS 可缺（没配就提示 `--no-tts` 只出逐字稿，可用其他 TTS 工具二次合成）、素材存在（没掌握的课 / 错题串讲 / 题库任一有货）。
