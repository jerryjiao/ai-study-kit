...
2. **进流程**：按 [`../ask-coach/references/flows.md`](../ask-coach/references/flows.md) **「体检（doctor）」** 节执行——四门校验（品牌扫描 / 题库 QA / 测试 / 四对齐）+ 环境探测（LLM 三项、TTS、`:8787` 后端、主题目录、sync 产物新鲜度）+ 版本漂移与契约完整性两项探测（ADR-0006：用户项目 kit 版本 vs 插件快照、排布表/examPoint 标记/闪卡映射缺口——只报事实与补法，走 F13 补）逐项跑，按该节模板输出过/红汇总报告与**建议修复顺序**。
