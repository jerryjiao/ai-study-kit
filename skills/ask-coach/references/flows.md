...
- **考**按本站 mode 走（§6）：选择题即时判定 ✅/❌ + 一句理由（只给题面 + 选项，答完才解释）；复述与造按 §6.6 判定；**案例型考点**（DFD/ER/UML/算法/设计模式一类主观大题）走 §6.7 案例通道——真题风格原创大题 + 解题范式四步带练 + 评分点判定，判定进口头题计数，范式卡沉淀 `study/notes/`；定义错误零容忍，当场打断（§7）。
