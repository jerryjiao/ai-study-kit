...
## F10 · 现场串讲（walkthrough，对话式入门一个新主题）

**目的**：把一个新主题用「知识点一块一块讲、题一道一道答」当场学一轮。对话式教学——区别于 F6（产持久课程 HTML）和 F4（过错题）。方法论闭环的对话形态：**大纲定讲什么 → 材料讲概念 → 当场做题验效果**。

**前置（事实核查，开讲前必做——凭记忆开讲是大忌）**：

1. **深度权威**：主题 MISSION.md 的考点排布表——讲到什么深度、什么题型，表说了算；没排布表先按 F2 第 1-2 步补，或声明按材料自拟大纲。
2. **讲哪些**：questions.json 按主题筛题，对拟讲考点做词频统计——高频考点优先，冷门考点大胆跳过。
3. **概念来源**：主题源目录 `lessons/*.html` 与 RESOURCES.md。讲概念引材料原文（文件+位置）；材料没覆盖的直说「材料未覆盖」，类比可以给但不冒充权威。
4. **用户状态**：Step 1 快照/`/api/progress` 看该主题正确率——90%+ 走快节奏；已有错题串讲（F4 产物）必读，用户的真实痛点考点别装覆盖。
5. **避免重复**：主题 `learning-records/`（如有）已有该主题记录，先看上次覆盖了什么。

**教学纪律（实战提炼，逐条硬约束）**：

- 主题拆 **6-12 个考点**；一次只讲一个（≤200 字 + 1 表/图），讲完**不预告下一个**。
- 每个考点一句**锚点金句**（可背死的短句，如「指标 = 维度 + 度量」）。
- 抽象概念配**生活类比**，并列多项给**助记口诀**——这是标配不是附加；材料里有就引原文，自造的标明「自造助记」。
- 每考点**连出 2-3 道题**，但**一道一道提交**：Q1 定义 → Q2 辨析 → Q3 应用，全验完才进下一考点。
- 出题**只给题面+选项**——无提示、无推荐、无元信息；答完才解释。
- **即时判定** ✅/❌ + 一句理由；错了点破踩的坑、重申锚点，再出**变体**（同考点不同题，不原题重答；自造变体先对照原题选项措辞）。
- 开场一段话告知范围（材料/题/课依据 + 考点骨架），然后**直接进考点 1**，不反问「想怎么学」。

**固化（串讲结束的同一回合主动做，不等用户问）**：

1. `learning-records/<NN>-<主题>-walkthrough.md`（目录无则建，本地学习记录）：考点骨架表 + 全部题目表 + 提炼的硬规则。
2. `wrong-questions/<主题>-walkthrough.html` + 更新 `wrong-questions/index.html` 入口卡。
3. **诚实定位（必须有）**：warn 框写明覆盖了什么 / 没覆盖什么（深度痛点指向 F4 串讲记录）——避免下次回看误以为「全过了」。
4. 新增 html 过 `pnpm run scan` 再 build。

**完成标志**：计划考点全讲完 + 最后考点答对 + 固化产物落盘 + scan 0 命中。接力：固化学过的主题去 F3 刷对应题集。
