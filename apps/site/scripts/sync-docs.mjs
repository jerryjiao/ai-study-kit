...
// 五语同步：docs/<name>.md（中文基准，挂站根）+ docs/<name>.<lang>.md（en/es/ru/ja 译本，挂 /<lang>/），
...
/** 每篇文档的页面 meta description（五语）：不写就用站点级兜底文案——28 个文档页曾被
...
const DESCRIPTIONS = {
  'methodology.md': {
    ...
    ja: '学習メソッド：能力アウトラインが何を問うかを定め、参考資料が概念を築き、演習が効果を検証する——三環がずれたら即修正する学習ループ。',
  },
  'four-alignment.md': {
    ...
    ja: '四つの整合の原則：コース・問題・フラッシュカード・間違えた問題の記録が同じ出題ポイントを軸に揃い、どこかを変えたら必ず双方向チェックを走らせる。',
  },
  'ai-cli-guide.md': {
    zh: '三个 AI 命令行工具：teach 产课程、grill 产错题精讲、podcast 产双播播客，支持五语输出与 --json 管道。',
    en: 'The three AI CLIs: teach generates lessons, grill writes wrong-answer deep-dives, podcast produces a two-host review show; five output languages and --json supported.',
    es: 'Las tres CLI de IA: teach genera lecciones, grill escribe repasos de errores y podcast produce un programa de repaso; salida en cinco idiomas y --json.',
    ru: 'Три ИИ-CLI: teach создаёт уроки, grill пишет разборы ошибок, podcast выпускает передачу; пять языков вывода и --json.',
    ja: '3 つの AI CLI：teach がコースを生成、grill が間違えた問題の徹底解説を書き、podcast が 2 人のナレーターの番組を生成。五言語の出力と --json パイプラインに対応。',
  },
  'ai-study-kit.md': {
    ...
    ja: '/ask-coach 学習コーチ：読み取り専用で状態を検出 → スナップショット＋おすすめ＋メニュー → 13 フローの playbook に沿って実行まで伴走。',
  },
  'configuration.md': {
    zh: '配置指南：在 .env 配 LLM/TTS provider——OpenAI 兼容端点、五语输出与语音合成。',
    en: 'Configuration guide: set your LLM/TTS providers in .env — OpenAI-compatible endpoints, five output languages and speech synthesis.',
    es: 'Guía de configuración: define tus proveedores LLM/TTS en .env — puntos compatibles con OpenAI, salida en cinco idiomas y síntesis de voz.',
    ru: 'Руководство по настройке: провайдеры LLM/TTS в .env — эндпоинты, совместимые с OpenAI, вывод на пяти языках и синтез речи.',
    ja: '設定ガイド：.env で LLM/TTS プロバイダを設定——OpenAI 互換エンドポイント、五言語の出力と音声合成。',
  },
  'bidirectional-check.md': {
    ...
    ja: '双方向チェックスクリプト：問題→コース、コース→問題、フラッシュカード網羅の三方向で四つの整合を自動チェック。',
  },
  'CHANGELOG.md': {
...
/** GitHub 用的语言切换栏（**简体中文** · [English](…) · … 五语互链行）站内剥掉。
...
function render({ src, dest, order, fromRoot, title: titleOverride, lang = 'zh' }) {
  ...
  text = text.replace(/\((\.?\/)?([A-Za-z-]+(?:\.(?:en|es|ru|ja))?\.md)\)/g, (m, _prefix, file) =>
    map[file] ? `(${map[file]})` : m);
