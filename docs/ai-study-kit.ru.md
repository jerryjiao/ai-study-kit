...
pnpm run skill:install          # копирует в ~/.agents/skills/{ask-coach,coach,doctor,recap}
...
Плюс два эксплуатационных входа: **проверка здоровья** (`/doctor` — комплексная оркестрация четырёх ворот качества + зондов окружения, с отчётом «прошёл/красный» и порядком починки) и **диагностика** (прогресс не синхронизируется, курс отдаёт 404, CLI ругается на конфигурацию, scan что-то нашёл… — таблица «симптом → корень → действие»).
