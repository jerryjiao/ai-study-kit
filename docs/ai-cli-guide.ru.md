...
Инструмент-спутник grill: **детерминированно выводит** владение каждым пунктом экзамена (`examPoint` вопроса, EP-NN) из банка вопросов + прогресса ответов, без LLM. Общий для людей и агентов: люди читают таблицу, агенты потребляют `--json` (строка «слабые пункты» в снимке состояния `/ai-study-kit` берётся отсюда).

### Критерии (четыре состояния)

| Состояние | Критерий |
|-----------|----------|
| освоено | все вопросы пункта отвечены, все верны на последней попытке, невыпущенных ошибок нет |
| слабо | есть невыпущенные ошибки или неверный ответ на последней попытке |
| в процессе | отвечена часть, негативных признаков нет |
